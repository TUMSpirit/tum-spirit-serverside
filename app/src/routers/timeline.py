...
@router.get("/timeline/insert", tags=["timeline"])
...
# Define a route to insert a record into the database
@router.put("/timeline/update", tags=["timeline"])
def update_timelineEntry(timeline_id, newValues):
    try:
        query = { '_id': timeline_id }
...
# Define a route to insert a record into the database
@router.put("/timeline/delete", tags=["timeline"])
def insert_timelineEntry(timeline_id):
    try:
        # Create a record with a random ID (ObjectId) and a timestamp
        query = { '_id': timeline_id }
        # Inserting the record into the database
